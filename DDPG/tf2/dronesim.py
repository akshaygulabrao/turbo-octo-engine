...
class DroneSim(gym.Env):
    ...
    def step(self, action):
        ...
        # )
        self.agent_location += self.agent_velocity * 10
        action = action.numpy()
        action[np.abs(action) < 0.5] = 0
        self.agent_velocity += action * 0.1
        self.agent_velocity = np.clip(
            self.agent_velocity, -10,10
        )
        # print(self.agent_location,self.agent_velocity,action)
        ...
        if (self.agent_location < 0).any() or (self.agent_location > self.window_size).any():
            reward = -20000
            terminated = True
        elif np.linalg.norm(self.target_location - self.agent_location) < 10:
            reward =1
            terminated = True
        else:
            reward = -1 * (np.linalg.norm(self.target_location - self.agent_location)**2)
            terminated = False
        observation = self._get_obs()
        info = self._get_info()
    ...
    def _render_frame(self):
        ...
        pygame.draw.circle(
            canvas,
            (0, 0, 255),
            (self.agent_location.astype(int) + 0.5),
            10
        )
